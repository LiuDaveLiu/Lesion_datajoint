...
def get_schema_name(name):
    return 'group_shared_lesion-'+name
...
def drop_every_schema():
 #%% Drop everything   
 #%%
    schema = dj.schema('group_shared_lesion-behavior-anal')
    schema.drop(force=True) 
     #%%
    schema = dj.schema('drop_every_schema-ephys-anal')
    schema.drop(force=True)
    #%%
    schema = dj.schema('drop_every_schema-ephys_patch')
    schema.drop(force=True)
    #%%
    schema = dj.schema('drop_group_shared_lesion-experiment')
    schema.drop(force=True)
    #%%
    schema = dj.schema('drop_every_schema-lab')
    schema.drop(force=True) 
